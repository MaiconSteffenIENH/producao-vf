...
6. **Commite sempre, sem eu pedir.** Trabalho validado e não commitado é trabalho que eu esqueço e perco. Terminou e passou no `validar.sh`? Commita. O commit nunca fica para depois.
7. **Termine a resposta com o comando de git que sobrou para mim.** Sempre que houver commit esperando push, ou for preciso um pull, a última linha da resposta é o comando pronto para copiar, com a contagem do que está pendente. Sem isso eu fecho a conversa e o trabalho fica parado na minha máquina.
8. **Tudo em pt-BR**, e explicação breve: código, comentário, commit, UI e mensagem de erro.
9. **Nome de cadastro é CAIXA ALTA** (peça, esmalte, categoria, etapa, responsável). Já sobe ao digitar; `npm run caixa-alta` arruma o histórico.
10. **Dinheiro não entra no cadastro de peça.** Quem cadastra é a Gabi, e valor não é assunto dela — preço mora na tela de Preços.
11. **O `.env` aponta para produção (Neon).** Nunca `prisma migrate dev` nele, só `migrate deploy`; migração escrita à mão quando o CLI não puder rodar.
12. **Push dispara deploy** (Render e Vercel). O app é PWA: depois do deploy pode ser preciso Cmd+Shift+R para sair do cache.
13. **Mudança que mexe no livro-razão passa por revisão adversarial** antes de ir. Ela já derrubou três versões que pareciam prontas.
...
10. **O forno é CARGA, não etapa.** Queima junta peças de vários lotes, ocupa volume fixo e trava o forno ~2 dias. Peça não espera o forno — espera o forno **encher**. Daí sai a sugestão que nenhum ateliê calcula de cabeça: "faltam 12 para fechar a carga, e essas 12 adiantam as outras 68". A capacidade vem de `Etapa.capacidadeCarga` **da própria etapa de queima**, não de um responsável: forno não é gente nem equipe, e a pergunta que se faz é "cabe mais NESTA queima?". Os responsáveis `tipo = forno` que restam no banco são resquício da modelagem antiga, mantidos inativos só para o histórico que os cita continuar legível — o cadastro de Responsáveis já não oferece esse tipo. Duas paradas de forno no roteiro não são dois fornos: são o mesmo forno em dois momentos, e biscoito × esmalte se decide pela posição em relação à etapa que define a cor — nunca pelo nome nem por número de ordem cravado.
...
Nada em `backend/src/lib/` importa Prisma, de propósito — é o que permite testar a matemática do sistema sem subir banco (`npm run test:unidade`, 494 casos em 26 arquivos, ~2s). Regra nova que seja calculável a partir dos dados de entrada nasce aqui, não dentro do service.
